...
TEST_CASE(VFormat_DirectCharacters_HappyCase)
{
	...
	size_t size = RLM_Format(buffer, sizeof(buffer), "Hello World");
...
TEST_CASE(VFormat_DirectCharacters_NULL)
{
	...
	size_t size = RLM_Format(buffer, sizeof(buffer), NULL);
...
TEST_CASE(VFormat_DirectCharacters_EscapeCharacters)
{
	...
	size_t size = RLM_Format(buffer, sizeof(buffer), "27%%");
...
TEST_CASE(VFormat_DirectCharacters_Truncate)
{
	...
	size_t size = RLM_Format(buffer, 8, "Hello World");
...
TEST_CASE(VFormat_DirectCharacters_TruncateNoNul)
{
	...
	size_t size = RLM_FormatNoNul(buffer, 8, "Hello World");
...
TEST_CASE(VFormat_FormatCharacter_HappyCase)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%c%c%c", 'A', 'B', 'C');
...
TEST_CASE(VFormat_FormatCharacter_SpecialCharacters)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%c%c%c%c", '\\', '\r', '\n', 0xD7);
...
TEST_CASE(VFormat_FormatString_HappyCase)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%s", "ABC");
...
TEST_CASE(VFormat_FormatString_NULL)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%s", g_our_private_null_ptr);
...
TEST_CASE(VFormat_FormatString_SpecialCharacters)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%s", "Hello World\\\r\n\xD7");
...
TEST_CASE(VFormat_FormatUInt_HappyCase)
{
	...
	for (uint32_t test_case : TEST_CASES)
	{
		...
		RLM_Format(buffer, sizeof(buffer), "%u", test_case);
...
TEST_CASE(VFormat_FormatInt_HappyCase)
{
	...
	for (int32_t test_case : TEST_CASES)
	{
		...
		RLM_Format(buffer, sizeof(buffer), "%d", test_case);
...
TEST_CASE(VFormat_FormatHexUpper_HappyCase)
{
	...
	for (uint32_t test_case : TEST_CASES)
	{
		...
		RLM_Format(buffer, sizeof(buffer), "%X", test_case);
...
TEST_CASE(VFormat_FormatFloat_HappyCase)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f", -123.456, M_PI);
...
TEST_CASE(VFormat_FormatFloat_NotANumber)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f", -std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN());
...
TEST_CASE(VFormat_FormatFloat_Infinity)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f", -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity());
...
TEST_CASE(VFormat_FormatFloat_Zero)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f", -0.0, 0.0);
...
TEST_CASE(VFormat_FormatFloat_Exponents)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f %f %f", 9999999999.0, 10000000000., 0.000011, 0.00001);
...
TEST_CASE(VFormat_FormatFloat_Rounding)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f", 0.9999994999, 0.9999995000);
...
TEST_CASE(VFormat_FormatFloat_Range)
{
	...
	RLM_Format(buffer, sizeof(buffer), "%f %f %f %f", 4.9406564584124654e-324, 2.2250738585072009e-308, 2.2250738585072014e-308, 1.7976931348623157e308);
